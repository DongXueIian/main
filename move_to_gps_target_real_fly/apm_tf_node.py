...
class TFConverterNode(Node):
    ...
    def target_gps_callback(self, msg):
        # if self.target_gps!=None:
        #     print('target_gps_callback'+' '+str(msg.latitude)+' '+str(self.target_gps.latitude)+' '+str(msg.longitude)+' '+str(self.target_gps.longitude)+' ')
        if self.target_gps==None or self.target_gps.latitude!=msg.latitude or self.target_gps.longitude!=msg.longitude:
            self.gps_changed=True
            self.target_gps=msg
            # print('self.target_gps=msg')
        # print('target_gps    '+str(msg))

    def home_gps_callback(self, msg):
        # print('home_gps_callback')
        if self.home_gps==None or self.home_gps.latitude!=msg.latitude or self.home_gps.longitude!=msg.longitude:
            self.home_gps=msg
            self.gps_changed=True
    ...
    def update_1hz(self):
        # print('update_1hz')
        if self.gps_changed and self.target_gps!=None and self.home_gps!=None:
